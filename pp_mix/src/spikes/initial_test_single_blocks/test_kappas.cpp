...
int main() {
    ...
    Lambda.block(0,0,3,1) = VectorXd::Ones(3);
    Lambda.block(3,1,3,1) = - VectorXd::Ones(3);
...
*/
    ...
    std::cout<<"kappas: \n"<<Kappas<<std::endl;
    std::cout<<"kappasTronc: \n"<<Kappas.bottomRows(Kappas.rows()/(2*pp_mix->get_N() +1) * (pp_mix->get_N() +1))<<std::endl;
