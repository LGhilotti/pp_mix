...
if (chi < ZTOL) {
  ...
  if (lambda > 0.0) {
    res = gamma_rng(lambda, psi/2.0, Rng::Instance().get());
  }
  else {
    res = inv_gamma_rng(-lambda, psi/2.0, Rng::Instance().get());  // ???  vedere foglio (a)
  }
}
  ...
  //  for (i=0; i<n; i++) res[i] = 1.0/R::rgamma(lambda, 2.0/chi); // ??? vedere foglio (b)
    ...
    res = inv_gamma_rng(-lambda, chi/2.0, Rng::Instance().get());
  }
  ...
  do {
    if (lambda > 2. || omega > 3.) {
      /* Ratio-of-uniforms with shift by 'mode', alternative implementation */
      _rgig_ROU_shift_alt(res, lambda, lambda_old, omega, alpha);
      break;
    }

    if (lambda >= 1.-2.25*omega*omega || omega > 0.2) {
      /* Ratio-of-uniforms without shift */
      _rgig_ROU_noshift(res, lambda, lambda_old, omega, alpha);
      break;
    }

    if (lambda >= 0. && omega > 0.) {
      /* New approach, constant hat in log-concave part. */
      _rgig_newapproach1(res, lambda, lambda_old, omega, alpha);
      break;
    }
