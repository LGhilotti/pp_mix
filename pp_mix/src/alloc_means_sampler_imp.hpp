...
T MeansSamplerMala::alloc_means_target_function::operator()(const Eigen::Matrix<T,Eigen::Dynamic,1> & mean) const {
  ...
  MatrixXd others(allmeans.rows() - 1, m_mcmc.get_dim_fact());
  others = delete_row(allmeans, ind_mean);

  T output{0.};
  
  int n = m_mcmc.get_etas_by_clus(ind_mean).size();

  MatrixXd cluster_etas(m_mcmc.get_dim_fact(), n );
  for (int i = 0; i < n ; i++)
    cluster_etas.col(i) = m_mcmc.get_etas_by_clus(ind_mean)[i];
  
  output -= 0.5 * sum(multiply(m_mcmc.get_single_a_delta(ind_mean).get_cho_factor_eval(), (cluster_etas.colwise()-mean)).colwise().squaredNorm());

  
  MatrixXd others_trans(allmeans.rows() - 1 ,m_mcmc.get_dim_fact());
