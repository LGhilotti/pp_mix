...
void ConditionalMCMC<Prec, prec_t, data_t>::sample_means() {
  ...
  //   VectorXd bias_curr = grad_curr.array() * sigma * stepsize;
  //   VectorXd prop_mean = currmean + bias_curr;

  //   VectorXd prop = VectorXd::Zero(prop_mean.size());
  //   for (int k = 0; k < prop.size(); k++)
  //     prop(k) = trunc_normal_rng_inversion(prop_mean(k), sigma, ranges(0, k),
  //                                          ranges(1, k), Rng::Instance().get());

  //   VectorXd grad_prop = compute_grad_for_clus(i, prop);
  //   VectorXd bias_prop = grad_prop.array() * sigma * stepsize;
  ...
  //       // multi_normal_cholesky_lpdf(currmean, mean_proposal_inverse,
  //       //                            proposal_var_chol);
